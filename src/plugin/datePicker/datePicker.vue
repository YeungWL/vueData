    ...
    <div class="container">
      <label class="input-label">
        ...
        <span class="iconfont icon-el-icon-circle-close"
              v-show="!!selectedDate"
              @click.prevent="selectedDate = ''"></span>
      </label>
      <div class="date-container" :class="{'container-visible': containerVisible}" @mousedown.prevent>
        <div class="date-header">
          <button type="button" class="btn last-year-btn" @mousedown="changeMonth(-12)">
            <span class="iconfont icon-el-icon-d-arrow-left"></span>
          </button>
...
export default {
  ...
  methods: {
    // 改变月份
    changeMonth(count) {
      const changeDate = new Date(this.year, this.month + count - 1);
      this.datePicker.changeDate(changeDate);
    },
    // 选择日期
    selectDate(date) {
